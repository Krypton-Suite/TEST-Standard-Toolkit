...
public static class KryptonExceptionDialog
{
    ...
    /// <param name="githubConfigPath">Optional path to the encrypted config file. If null, the default path is used.</param>
    private static void ShowCore(Exception exception, Color? highlightColor, bool? showCopyButton, bool? showSearchBox, Action<Exception>? bugReportCallback, SecureString? githubSecretKey, string? githubConfigPath) =>
        VisualExceptionDialogForm.Show(exception, highlightColor, showCopyButton, showSearchBox, bugReportCallback, githubSecretKey, githubConfigPath);
