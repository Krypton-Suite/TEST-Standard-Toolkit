...
public partial class TooltipTimeoutTest : KryptonForm
{
    ...
    private void ApplyTimeoutMode()
    {
        ...
        kryptonToolTip1.AutoPopDelay = autoPopDelay;

        // Update KryptonToolTip tooltips for demo controls
        UpdateKryptonToolTipTexts();
    ...
    private void UpdateKryptonToolTipTexts()
    {
        int delay = GetSelectedAutoPopDelay();
        string suffix = delay == 0
            ? " This tooltip will stay visible until you move the pointer away."
            : $" This tooltip will hide after {delay / 1000} seconds.";

        kryptonToolTip1.SetToolTip(kbtnDemoButton, $"KryptonToolTip (AutoPopDelay={delay}ms){suffix}");
        kryptonToolTip1.SetToolTip(ktxtDemoInput, $"Text input with KryptonToolTip{suffix}");
        kryptonToolTip1.SetToolTip(kchkDemoCheck, $"Checkbox with KryptonToolTip{suffix}");
    }
