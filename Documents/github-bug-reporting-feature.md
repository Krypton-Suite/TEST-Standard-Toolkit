...
| `IsValid` | `bool` | `true` if Owner, RepositoryName, and PersonalAccessToken are all non-empty |
...
Static class providing the user-facing bug report dialog. Loads config from the encrypted file (or accepts config directly) and shows a form with Title and Description fields.
